...
class CalendarMain extends Component {
    ...
    handleDatePick(index, styleName, stock, isCanPick) {
        ...
        if (!isCanPick && styleName == "thisMonth") { return }
        switch (styleName) {
            case 'thisMonth':
                                ...
                                <tr key={rowIndex}>
                                    {
                                        row.map((day) => {
                                            ...
                                            let showClass = (this.props.selcetedDays.indexOf(day) == -1) ? '' : 'picked';
                                            showClass = styleOfDays[i] == "thisMonth" ? showClass : '';
                                            let isPointer = this.props.stocks[day - 1] > 0 ? "pointer" : "";
                                            {/* 小于今天不让点 */ }
                                            let isBeforeToday;
                                                ...
                                                <td className={styleOfDays[i] + ' ' + showClass + ' ' + isPointer + ' ' + isCanPick}
                                                    onClick={
                                                        this.handleDatePick.bind
                                                            (this, i, styleOfDays[i], this.props.stocks[day - 1], isCanPick)}
                                                    key={i++}>
                                                    {day}
