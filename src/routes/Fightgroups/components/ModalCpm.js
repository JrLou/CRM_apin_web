...
class SendLogModal extends Component {
  ...
  getDataSource(data) {
    ...
    return data.map(item => {
      const depFlightInfo = item.flightInfo
        ? item.flightInfo.filter(oneFlight => oneFlight.trip_index === 0)[0]
        : {};
      const arrFlightInfo = item.flightInfo
        ? item.flightInfo.filter(oneFlight => oneFlight.trip_index === 1)[0]
        : {};
      return {
        ...item,
...
class ExportPassengerModal extends Component {
    ...
    if (abroad == 0) {
      ...
      columns = [
        ...
        },
        {
          ...
          dataIndex: "ticket",
          render: text => (
            <Spin spinning={this.state.ticketLoading}>{text}</Spin>
          ),
        },
      ];
    } else {
  ...
  getPaidMemberAfterInsertTickets(newData, oldData, abroad) {
    ...
    const resultArr = oldData.map((currV, index) => {
      ...
      return {
        ...
        ticket:
          newData[index + 1] &&
          newData[index + 1][abroad === 0 ? 4 : 8] &&
          newData[index + 1][abroad === 0 ? 4 : 8].toString(),
      };
    });
    return resultArr;
  }
  ...
  isTicketChange(newData, oldData) {
    return oldData.some(
      (currV, index) => currV.ticket != newData[index].ticket
    ); //为了匹配defined == null，所以不能用【全等】
  }
        ...
        this.setTicketLoading(true, () => {
          ...
          if (file.status === "done") {
            ...
            if (file.response.code >= 1) {
              paidMemberAfterInsertTicket = this.getPaidMemberAfterInsertTickets(
                file.response.data,
                data,
                abroad
              );
              isTicketChange = this.isTicketChange(
                paidMemberAfterInsertTicket,
                data
              );
              if (isTicketChange) {
                dispatch({
