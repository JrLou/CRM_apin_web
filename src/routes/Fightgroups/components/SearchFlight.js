...
export default class SearchFlight extends PureComponent {
  ...
  addFlight = (values) => {
    const { data: { isLeft } } = this.props;
    let inputId = isLeft ? 'startDate' : 'endDate';
    let date = this.props.form.getFieldValue(inputId);
    // 拼接成飞常准的数据格式
    values.FlightDeptimePlanDate = moment(date).format('YYYY-MM-DD') + ' ' + values.FlightDeptimePlanDate;
    values.FlightArrtimePlanDate = moment(date).format('YYYY-MM-DD') + ' ' + values.FlightArrtimePlanDate;
    this.props.dispatch({
      type: 'push/setCard',
      payload: [values],
    });
