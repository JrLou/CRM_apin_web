      ...
      for (let i = 0; i < this.passengerData.length; i++) {
        ...
        if (this.orderData.group_type != 3) {
          ...
          if (!user.ticketDep && !user.ticketArr) {
            ...
            ticket = user.ticketDep + ',' + user.ticketArr;
          }
        } else {
          ticket = user.ticketDep;
        }
          ...
          return (<span>
            {
              (nameType == 'FlyingPig' && order_status == 2) || (nameType == 'Entrust' && order_status == 4) ?
                <span>
                  ...
                  返
                  <Input className={styles.inputTicket} disabled={order.group_type == 3 ? true : false}
                         onChange={(e) => {
